...
void InstanceImportTask::processPackwiz()
{
    ...
    // import, before the pack.toml had been fetched) now that we know the pack's real name. No
    // version suffix - just the pack name, unlike Modrinth/Flame's "name + version" convention.
    // If the user explicitly typed a different name, keep that - but modifiedName() alone can't
    // tell an explicit override apart from the unset placeholder (it falls back to
    // originalName()), so compare the two directly.
    QString explicitOverride = modifiedName() != originalName() ? modifiedName() : QString();

    InstanceName real_name(m_packwizPack.name, QString());
